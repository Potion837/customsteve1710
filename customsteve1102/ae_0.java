 ...
 * Renamed from customsteve.aE
 */
public class ae_0
extends ad_0 {
    public ae_0(ResourceLocation par1ResourceLocation) {
    ...
    public void d() throws IOException {
        ...
        try {
            ...
            BufferedImage bufferedimage = ae_0.a(inputstream);
            boolean flag = false;
            boolean flag1 = false;
            TextureUtil.uploadTextureImageAllocate((int)this.a(), (BufferedImage)bufferedimage, (boolean)flag, (boolean)flag1);
        }
        finally {
            if (inputstream != null) {
                inputstream.close();
            ...
            if (imageType == 0) {
                ...
                if (ltr && btt) {
                    for (y = height - 1; y >= 0; --y) {
                        for (x = 0; x < width; ++x) {
                            ae_0.a(bytes, (y * width + x) * 4, pixelDepth, stream);
                        }
                    }
                } else if (!ltr && btt) {
                    for (y = height - 1; y >= 0; --y) {
                ...
                if (ltr && btt) {
                    for (y = height - 1; y >= 0; --y) {
                        for (x = 0; x < width; ++x) {
                            ae_0.a(bytes, (y * width + x) * 4, stream);
                        }
                    }
                } else if (!ltr && btt) {
                    for (y = height - 1; y >= 0; --y) {
                    ...
                    for (y2 = height - 1; y2 >= 0; --y2) {
                        for (x2 = 0; x2 < width; ++x2) {
                            if (skip > 0) {
                                ...
                                ae_0.a(bytes, (y2 * width + x2) * 4, pixelDepth, stream);
                                --skip;
                                continue;
                            }
                            i3 = (y2 * width + x2) * 4;
                            result = ae_0.a(bytes, i3, pixelDepth, stream);
                            onRLE = result[0] == 1;
                            skip = result[1];
                            ae_0.a(bytes, i3, pixelDepth, stream);
                            --skip;
                            cR = bytes[i3];
                            cG = bytes[i3 + 1];
                            cB = bytes[i3 + 2];
